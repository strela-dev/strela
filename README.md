...
```sh
make docker-build docker-push IMG=<some-registry>/strela:tag
```
...
```sh
make deploy IMG=<some-registry>/strela:tag
```
...
```sh
make build-installer IMG=<some-registry>/strela:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/strela/<tag or branch>/dist/install.yaml
```
